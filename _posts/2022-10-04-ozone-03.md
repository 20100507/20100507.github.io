...
```
   大数据对实时性数据要求越来越高,以往的拉姆达n+1的架构，在很多业务场景中已经不再适用。近几年湖仓一体被提上日常、其中数据湖为最重要的一环、
   譬如iceberg、hudi、delta lake诸多优秀的数据湖框架应用而生。数据湖可以做到近实时、延迟可以降低到分钟级别。
   
   但是数据湖适用过程中小文件是系统最头疼的一个环节、众所周知小文件对于HDFS是最大的诟病。因此对象存储如雨后春笋一般大量涌现、
   先后有腾讯云对象存储、百度云对象存储、阿里云等等。这些均为云厂商、社区可扩展对象存储并不多
   、其中以Ozone(HDDS),由Hadoop原创团队+腾讯研发团队共同维护，基于Apache的背景、前途发展非常好。
